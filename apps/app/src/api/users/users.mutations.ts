...
async function updateUser(id: string, data: UpdateUserData) {
  const res = await api.users[":id{[a-zA-Z0-9_-]{21}}"].$put({
    param: { id },
    json: data,
  });
...
async function deleteUser(id: string) {
  const res = await api.users[":id{[a-zA-Z0-9_-]{21}}"].$delete({
    param: { id },
  });
...
async function resetUserPassword(id: string) {
  const res = await api.users[":id{[a-zA-Z0-9_-]{21}}"]["reset-password"].$post({
    param: { id },
  });
...
async function impersonateUser(id: string) {
  const res = await api.auth.impersonate[":id{[a-zA-Z0-9_-]{21}}"].$post({
    param: { id },
  });
...
async function updateUserRoles(id: string, roleIds: number[]) {
  const res = await api.users[":id{[a-zA-Z0-9_-]{21}}"].roles.$put({
    param: { id },
    json: { roleIds },
  });
