...
export async function fetchUserSessions(userId: string) {
  const res = await api.users[":id{[a-zA-Z0-9_-]{21}}"].sessions.$get({
    param: { id: userId },
  });
...
export async function revokeUserSession(userId: string, tokenId: string) {
  const res = await api.users[":id{[a-zA-Z0-9_-]{21}}"].sessions[":tokenId"].$delete({
...
export async function revokeAllUserSessions(userId: string) {
  const res = await api.users[":id{[a-zA-Z0-9_-]{21}}"].sessions.$delete({
    param: { id: userId },
  });
