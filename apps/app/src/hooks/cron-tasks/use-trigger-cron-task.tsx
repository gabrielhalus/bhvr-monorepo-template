  ...
  return useMutation({
    ...
    onSuccess: (data, taskId) => {
      ...
      if (data.success && data.run.status === "success") {
        toast.success(t("cron.actions.triggerSuccess"));
      } else if (data.success && data.run.status === "error") {
        toast.error(`${t("cron.actions.triggerError")}: ${data.run.error ?? "Unknown error"}`);
      }
    },
    meta: { errorMessage: t("cron.actions.triggerError") },
  });
}
