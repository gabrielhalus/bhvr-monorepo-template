...
export function UserInformationsForm({ userId }: { userId: string }) {
  ...
  return (
    <FormSection
      ...
      sub={t("account.sections.profile.description")}
    >
      <form
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        <div className="p-5">
          <FieldGrid>
            <form.Field
              name="firstName"
              children={field => (
                <Fg label={t("account.fields.firstName")} htmlFor={field.name} req>
                  <Input
                    id={field.name}
                    name={field.name}
            ...
            <form.Field
              name="lastName"
              children={field => (
                <Fg label={t("account.fields.lastName")} htmlFor={field.name} req>
                  <Input
                    id={field.name}
                    name={field.name}
                    value={field.state.value}
            ...
            <form.Field
              name="email"
              children={field => (
                <Fg label={t("account.fields.email")} htmlFor={field.name} req span2>
                  <Input
                    id={field.name}
                    name={field.name}
                    value={field.state.value}
        ...
        <form.Subscribe
          ...
          children={([canSubmit, isSubmitting, isDefaultValue]) => (
            <SecFoot note={t("account.sections.profile.note")}>
