...
export const Route = createFileRoute("/_auth/register/")({
  ...
  beforeLoad: async ({ context }) => {
    // During first-run setup (no user yet) registration is always available so
    // the initial system administrator can be created.
    const { needsSetup } = await context.queryClient.ensureQueryData(setupStatusQueryOptions);
    if (needsSetup) {
      return { needsSetup };
    }

    const { value: config } = await context.queryClient.ensureQueryData(configQueryOptions("authentication.register.enable"));
    ...
    <div className="flex min-h-svh bg-paper">
      ...
      <div className="relative hidden flex-col justify-between overflow-hidden bg-ink p-12 text-paper lg:flex lg:w-[45%]">
        ...
        <div className="relative z-10 space-y-5">
          ...
          <h2 className="text-sheen text-[2.6rem] font-extrabold leading-[1.1] tracking-[-0.03em]">
            {needsSetup ? t("register.setup.heroTitle") : "Rejoignez l'équipe."}
          </h2>
          <p className="max-w-xs text-base/relaxed text-white/55">
            {needsSetup ? t("register.setup.heroSubtitle") : "Créez votre compte pour accéder au tableau de bord de gestion."}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col items-center justify-center p-8">
        ...
        <div className="w-full max-w-sm">
          <RegisterForm needsSetup={needsSetup} />
        </div>
      </div>
    </div>
  );
